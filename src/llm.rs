...
use crate::codex_auth::{
    codex_config_default_openai_base_url, is_codex_app_server_provider, is_openai_codex_provider,
    is_qwen_portal_provider, refresh_openai_codex_auth_if_needed, resolve_openai_codex_auth,
    resolve_qwen_portal_auth,
};
...
pub fn create_provider(config: &Config) -> Box<dyn LlmProvider> {
    if is_codex_app_server_provider(&config.llm_provider) {
            ...
            codex_app_server_write_result(
                ...
                json!({
                    "contentItems": [
                        {
                            "type": "inputText",
                            "text": "Dynamic tool calls are disabled for codex-app-server LlmProvider requests."
                        }
                    ],
                    "success": false,
                }),
